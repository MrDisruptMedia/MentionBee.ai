    ...
    <>
      ...
      <article className="mx-auto w-full max-w-3xl px-4 pb-20 pt-24 md:pt-28">
        <header className="mb-10">
          <p className="text-sm font-medium uppercase tracking-wide text-mention-dark">MentionBee Guide</p>
          <h1 className="mt-3 font-heading text-3xl font-bold tracking-tight text-mention-dark sm:text-4xl">
            {content.title}
          </h1>
        </header>

        <div className="prose prose-zinc max-w-none prose-headings:font-heading prose-a:text-mention-dark">
          ...
          <nav className="mt-12 rounded-2xl border border-zinc-200 bg-zinc-50 p-6" aria-label="Weiterführende Links">
            ...
            <ul className="mt-4 space-y-2">
              {content.internalLinks.map((href) => (
                <li key={href}>
                  <Link href={href} className="font-medium text-mention-dark underline underline-offset-2 hover:text-mention-gray">
                    {href}
                  </Link>
                </li>
              ))}
