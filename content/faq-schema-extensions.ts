/** Extra FAQPage mainEntity entries merged into homepage FAQ JSON-LD (plain text only). */
export const faqHomepageSchemaExtensions = [
  {
    question: "Was ist AI Visibility?",
    answer:
      "AI Visibility beschreibt, wie prominent und positiv eine Marke in Antworten generativer KI-Systeme wie ChatGPT, Claude, Gemini und Perplexity erscheint. Anders als bei klassischem SEO geht es nicht um Rankings, sondern darum, ob eine Marke erwähnt, eingeordnet und empfohlen wird, wenn potenzielle Kunden KI-Assistenten kaufnahe Fragen stellen.",
  },
  {
    question: "Welche KI-Systeme analysiert MentionBee?",
    answer:
      "MentionBee analysiert ChatGPT, Claude, Gemini und Perplexity. Diese vier Systeme decken den Grossteil der KI-gestützten Recherche- und Kaufentscheidungen ab.",
  },
  {
    question: "Was kostet der AI Visibility Audit?",
    answer:
      "Der AI Visibility Audit kostet einmalig {{DEEP_DIVE_PRICE}}. Du erhältst einen individuellen Online-Report mit konkreten Massnahmen – kein Abo.",
  },
] as const;
