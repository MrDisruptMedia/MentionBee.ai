 ...
 *
 * Render order (ArticleExperience): contentType → tags → H1 → lead → author
 * meta → hero → markdown intro → structured TOC → remaining body. SoT bodies:
 * no duplicate H1/byline; short intro before `## In diesem Artikel`; then H2
 * sections without manual chapter numbers (TOC supplies numbering). Prefer LF.
 */
